...
Creating a `CITATION.cff` file with **cffr** is straightforward. You simply
...
In this case, we add `url`, `version`, and `repository`. We also overwrite
the `title` key. We add these arguments to `cff_modify()`:
