...
information for each of them. The details for each key are presented in the next
section of the document. The assessment of fields is based on the [Guide to
...
considered, however this can be modified via the `authors_roles` argument.
...
This key can be set via the arguments of the `cff_create()`/`cff_write()`
functions:
...
can retrieve the topics of the repository via the [GitHub
API](https://docs.github.com/en/rest) and include those topics as keywords. This
option is controlled via the `gh_keywords` argument:
...
without this field on the `DESCRIPTION` (which is the typical case for an
in-development package), **cffr** tries to search the package on any of the
