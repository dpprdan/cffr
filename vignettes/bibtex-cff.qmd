...
[^1]: By default **R** Pandoc would generate the cite in the Chicago author-date
    format [@rmarkdowncookbook2020]

:::: {#refs-example .references .csl-bib-body .hanging-indent data-entry-spacing="0"}
::: {#ref-einstein1921 .csl-entry}
```{r}
#| echo: false
#| results: asis
...
-   `cff_read_bib_text()` can read **BibTeX** entries that are already stored in
    a variable.
-   An S3 method `toBibtex.cff()` that converts from `cff` objects to Bibtex
...
Additionally, **series** would be ignored as there is no clear mapping on
...
Additionally, **series** and **type** would be ignored as there is no clear
...
prescribe an **author** field. On this cases, as [authors]{.underline} is
...
## Appendix A: **\@inbook** in BibTeX and BibLaTeX {.appendix}
...
## Appendix B: [CFF key:type]{.underline} values {.appendix}
